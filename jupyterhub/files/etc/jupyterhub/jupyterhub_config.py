...
from batchspawner import Ec2SlurmSpawner
from batchspawner.utils import get_ec2_option
from handlers.spawners import SaveSpawnerHandler
from handlers.stopwhilepending import StopServerHandler
from handlers.gethelp import ScipHelpHandler
from handlers.dcvauth import DcvAuthHandler, DcvSessionHandler

#------------------------------------------------------------------------------
# SciP customizations
#------------------------------------------------------------------------------
c.Scip.user_guides = '/shared/bundle/docs'

#------------------------------------------------------------------------------
c.JupyterHub.allow_named_servers = True

c.JupyterHub.ssl_key = '/root/key.pem'
c.JupyterHub.ssl_cert = '/root/cert.pem'
c.ConfigurableHTTPProxy.command = ['configurable-http-proxy', '--redirect-port', '80']
#JH default after login URL
c.JupyterHub.default_url = '/hub/home'

c.JupyterHub.spawner_class = Ec2SlurmSpawner

c.Spawner.http_timeout = 600
c.Spawner.start_timeout = 1200

c.JupyterHub.hub_ip=get_ec2_option('privateIp')
...
#SBATCH --constraint={{options}}
#SBATCH --partition={{partition}}
{% if runtime %}#SBATCH --time={{runtime}} {% else %}#SBATCH --time=8:0:0 {% endif %}
...
_term() { 
  ...
  dcv list-sessions | awk '{print $2}' | sed "s/'//g"| while read sessionid; do dcv close-session $sessionid; done
}
...
SCIP_JUPYTER_NB_PATH=/shared/bundle/jupyterhub/share/jupyter/Samples
test -d ${SCIP_JUPYTER_NB_PATH} && cp -rf -t . ${SCIP_JUPYTER_NB_PATH} || :

# Search for each anaconda installations in /shared folder, check their validity by searching for python file and install kernelspec to appear in Jupyterhub launcher for a current user
for entry in /shared/* ; do  [[ $entry =~ 'anaconda' ]] &&[ -f $entry/bin/python ] && $entry/bin/python -m ipykernel install --user --name=${entry:8} --display-name ${entry:8} ; done

# Remove staled kernels if any
for entry in /home/scip/$USER/.local/share/jupyter/kernels/* ; do jq -r '.argv[0]' $entry/kernel.json | xargs test ! -f && basename $entry | xargs jupyter kernelspec uninstall -f ; done

export PATH=/shared/bundle/singularity/bin:$PATH
which jupyterhub-singleuser
{% if srun %}{{srun}} --export=ALL,RSESSION_PROXY_RSTUDIO_1_4=1 {% endif %}{{cmd}}
...
c.PAMAuthenticator.admin_groups = set(
    filter(
        validate_group,
        os.getenv("JUPYTER_ADMIN_GROUPS", "jhubadmins").split(':')
    )
)
...
#  Default: 'http://:8000'
c.JupyterHub.bind_url = 'http://0.0.0.0:443'
...
c.JupyterHub.extra_handlers = [
    (r'/save/([^/]+)', SaveSpawnerHandler),
    (r'/save/([^/]+)/([^/]+)', SaveSpawnerHandler),
    (r'/stopserv', StopServerHandler),
    (r'/help', ScipHelpHandler),
    (r'/dcv/sessions/(?P<prefix>[^/]+)/(?P<username>[^/]+)/(?P<notebook>[^/]+)/', DcvSessionHandler),
    (r'/dcv/authenticate', DcvAuthHandler),
    ([r'/image/(.*)', web.StaticFileHandler, dict(path=c.Scip.user_guides + '/images')])
]
...
c.JupyterHub.services = [
    {
        ...
        'cwd': os.environ.get('JUPYTERHUB_SERVICES_PATHS', 'services') + '/slurm_proxy',
        'environment': {
            'ENVIRONMENT': 'production',
            'PCLUSTER_CONFIG_PATH': os.environ.get('PCLUSTER_CONFIG_PATH',
                                                   '/opt/parallelcluster/configs/cluster_config.json'),
            'SLURM_API_URL': os.environ.get('SLURM_API_URL',
                                            'http://localhost:6800/slurm/v0.0.36/')
        },
        'command': [sys.executable, 'slurm_proxy.py']
    }
]
...
#  Default: []
# e.g. $ export JUPYTERHUB_TEMPLATES_PATHS=templates,/opt/jupyterhub/templates
templates_dirs = os.environ.get('JUPYTERHUB_TEMPLATES_PATHS', 'templates')
templates_dirs = [t.strip() for t in templates_dirs.split(',')]
c.JupyterHub.template_paths = templates_dirs
